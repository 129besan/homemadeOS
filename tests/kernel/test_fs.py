...
def test_initramfs_fileops_read(qemu_output):
    assert "initramfs fileops read" in qemu_output.lower()


def test_fdtable_read(qemu_output):
    assert "fdtable read" in qemu_output.lower()
